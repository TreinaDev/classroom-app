...
feature 'Customer see scheduled video class' do
  scenario 'successfully' do
    ...
    within("div\##{bodybuilding02.category}#{bodybuilding02.id}") do
      expect(page).to have_content('Musculacao - Aula 01')
      expect(page).to have_content(bodybuilding_teacher.name)
    end

    expect(html).not_to have_selector("div\##{crossfit01.category}#{crossfit01.id}")
    expect(html).not_to have_selector("div\##{bodybuilding01.category}#{bodybuilding01.id}")
